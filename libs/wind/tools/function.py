...
`wind/exceptions.py`) - which meant a tool raising `StopAgentRun(...)` to
...
from wind.metrics import ToolCallMetrics
from wind.models.response import ToolExecution
from wind.utils.json_schema import get_json_schema
...
class FunctionCall(BaseModel):
    ...
    def execute(self) -> ToolExecution:
        ...
        *deliberately* to signal the agent loop, not by accident - see
        `wind/exceptions.py` and `models/base.py:Model.run_function_call()`.
        """
