...
(or something says to stop early). Three helpers do the work:
    ...
    run_function_call()        one call's try/except - routes the control-flow exceptions
                                a tool can deliberately raise (see wind/exceptions.py)
...
from wind.exceptions import AgentRunException, RunCancelledException
from wind.metrics import MessageMetrics
from wind.models.message import Message
from wind.models.response import ModelResponse, ModelResponseEvent, ToolExecution
from wind.tools.function import Function, FunctionCall
from wind.utils.functions import get_function_call
from wind.utils.models.openai import format_tools_for_model as format_tools_canonical
...
class Model(ABC):
    ...
    def run_function_call(self, function_call: FunctionCall) -> ToolExecution:
        ...
        exceptions a tool can deliberately raise (`wind/exceptions.py`).
